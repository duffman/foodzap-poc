...
export interface INetworkConnector {
	name: string;
	onNewConnection(listener: (connection: IConnection) => void);
	onData(listener: (connection: IConnection, data: string) => void);
	onConnectionClosed(listener: (connection: IConnection, code: number, message: string) => void);
	onError(listener: (connection: IConnection, error: Error) => void);
}
