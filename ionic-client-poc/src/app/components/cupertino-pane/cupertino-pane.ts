...
export class CupertinoPane {
	...
	attachEvents() {
		...
		if (!Support.touch && Support.pointerEvents) {
			...
			if (Support.touch) {
				const passiveListener = this.touchEvents.start === 'touchstart' && Support.passiveListener && this.settings.passiveListeners ? {
					passive: true,
					capture: false
				} : false;
				this.paneEl.addEventListener(this.touchEvents.start, (t) => this.touchStart(t), passiveListener);
				this.paneEl.addEventListener(this.touchEvents.move, (t) => this.touchMove(t), Support.passiveListener ? {
					passive: false,
					capture: false
				} : false);
	...
	detachEvents() {
		...
		if (!Support.touch && Support.pointerEvents) {
			...
			if (Support.touch) {
				const passiveListener = this.touchEvents.start === 'onTouchStart' && Support.passiveListener && this.settings.passiveListeners ? {
					passive: true,
					capture: false
				} : false;
